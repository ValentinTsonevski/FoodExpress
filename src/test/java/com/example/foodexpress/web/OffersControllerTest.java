...
class OffersControllerTest {
    ...
    void testDeleteOfferById() {
        ...
        offerDto.setPrice(9.99);
        offerDto.setWeight(0.5);
        offerDto.setImage_url("https://example.com/test.jpg");
        Mockito.when(offerService.findById(1L)).thenReturn(offerDto);


        offerService.deleteOfferById(1L);

        Mockito.verify(offerService, Mockito.times(1)).deleteOfferById(1L);
    }
